...
<body class="bg-lib-bg-dark text-white font-display antialiased">
        ...
        <aside id="sidebar" data-lenis-prevent
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <button class="text-xs text-lib-primary hover:text-lib-primary-hover font-bold transition-colors">Clear
                        All</button>
                </div>
                ...
                <div class="space-y-3">
                    <button
                        class="w-full text-left px-4 py-2.5 bg-lib-surf-dark rounded-xl border border-white/10 hover:border-lib-primary/50 transition-all flex items-center justify-between group">
                        <span class="text-sm font-bold text-white">Categories</span>
                        <span
                            class="material-symbols-outlined text-gray-400 group-hover:text-lib-primary transition-colors">expand_more</span>
                    </button>
                    <div class="space-y-2 pl-2">
                        @foreach($categories as $category)
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox"
                                    class="w-4 h-4 rounded border-gray-600 text-lib-primary focus:ring-lib-primary focus:ring-offset-0">
                                <span
                    ...
                    <div class="space-y-4 px-2">
                        <div class="flex gap-3">
                            <input type="number" placeholder="Min"
                                class="w-1/2 px-3 py-2 bg-lib-surf-dark text-white text-sm rounded-lg border border-white/10 focus:border-lib-primary/50 focus:outline-none">
            ...
            <section class="sticky top-16 z-40 glass border-b border-white/10 px-6 lg:px-12 py-4">
                <div class="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
                    ...
                    <button id="mobileFilterToggle"
                        class="lg:hidden px-4 py-2 bg-lib-surf-dark rounded-xl border border-white/10 hover:border-lib-primary/50 transition-all flex items-center gap-2">
                        <span class="material-symbols-outlined text-xl">tune</span>
                        <span class="text-sm font-bold">Filters</span>
                    </button>

                    <!-- View Mode Toggle -->
                    <div class="flex items-center gap-2 bg-lib-surf-dark rounded-xl p-1 border border-white/10">
                        <button id="gridViewBtn"
                            class="px-3 py-2 bg-lib-primary text-white rounded-lg transition-all flex items-center gap-1.5">
                    ...
                    <div class="relative">
                        <select
                            class="px-4 py-2.5 bg-lib-surf-dark text-white text-sm font-semibold rounded-xl border border-white/10 focus:border-lib-primary/50 focus:outline-none appearance-none pr-10 cursor-pointer">
                ...
                <div class="max-w-7xl mx-auto">
                    @if($books->isEmpty())
                        <div class="p-20 text-center glass rounded-3xl border border-white/10">
                            <span
                                class="material-symbols-outlined text-8xl text-lib-primary mb-6 inline-block">library_books</span>
                        ...
                        <div id="booksGrid"
                            ...
                            @foreach($books as $book)
                                ...
                                <div class="book-card group relative" data-book-id="{{ $book->id }}"
                                    ...
                                    <div
                                        class="bg-lib-surf-dark rounded-xl md:rounded-2xl overflow-hidden border border-white/10 hover:border-lib-primary/50 transition-all duration-500 hover:shadow-2xl hover:shadow-lib-primary/20 flex flex-col h-full hover:-translate-y-1">

                                        <!-- Image -->
                                        <div
                                            class="relative aspect-[3/4] md:aspect-square overflow-hidden bg-gradient-to-br from-lib-surf-light to-lib-surf-dark">
                                            @if($book->cover_image)
                                            ...
                                            @else
                                                <div class="w-full h-full flex flex-col items-center justify-center text-gray-600">
                                                    <span
                                                        class="material-symbols-outlined text-4xl md:text-7xl mb-1 md:mb-3 transition-transform group-hover:scale-110 duration-500 text-lib-primary">auto_stories</span>
                                                    <span
                                            ...
                                            <div
                                                ...
                                                <span
                                                    class="hidden md:inline-block px-3 py-1.5 bg-gradient-to-r from-lib-primary/90 to-lib-secondary/90 backdrop-blur-md text-white text-xs font-black uppercase tracking-wide rounded-full">
                                            ...
                                            <div
                                                ...
                                                <button
                                                    class="quick-view-btn w-full py-2.5 bg-white text-gray-900 font-bold rounded-xl hover:bg-lib-primary hover:text-white transition-all flex items-center justify-center gap-2">
                                        ...
                                        <div class="p-2 md:p-4 flex flex-col flex-grow">
                                            <h3 class="text-white font-bold text-xs md:text-base mb-1 leading-tight line-clamp-2 group-hover:text-lib-primary transition-colors"
                                                title="{{ $book->title }}">
                                                {{ $book->title }}
                                            </h3>
                                            <p
                                                ...
                                                @if($book->type == 'rent' || $book->type == 'both')
                                                    <div>
                                                        <div
                                                            ...
                                                            Rent/Mo</div>
                                                        <div class="text-lib-primary font-bold text-xs md:font-black md:text-lg">
                                                ...
                                                @if($book->type == 'sale' || $book->type == 'both')
                                                    <div class="md:text-right">
                                                        <div
                                                            class="hidden md:block text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-0.5">
                                                            Buy</div>
                                                        <div class="text-lib-secondary font-bold text-xs md:font-black md:text-lg">
                                            ...
                                            <div class="grid grid-cols-1 gap-1.5 md:grid-cols-2 md:gap-2">
                                                @if($book->type == 'rent' || $book->type == 'both')
                                                    @if($book->quantity > 0)
                                                        @auth
                                                            <form action="{{ route('library.cart.add') }}" method="POST" class="w-full">
                                                                ...
                                                                <button type="submit"
                                                                    class="w-full py-1.5 px-2 md:py-2.5 md:px-3 rounded-lg md:rounded-xl bg-gradient-to-r from-lib-primary to-lib-primary-hover text-white font-bold text-[10px] md:text-xs hover:shadow-lg hover:shadow-lib-primary/40 transition-all flex items-center justify-center gap-1">
                                                                    <span>Rent</span>
                                                                    <span
                                                    ...
                                                    @else
                                                        <button disabled
                                                            class="w-full py-1.5 px-2 md:py-2.5 md:px-3 rounded-lg md:rounded-xl bg-lib-surf-light border border-white/10 text-gray-500 font-bold text-[10px] md:text-xs cursor-not-allowed flex items-center justify-center gap-1">
                                                    ...
                                                    @if($book->quantity > 0)
                                                        @auth
                                                            <form action="{{ route('library.cart.add') }}" method="POST" class="w-full">
                                                                ...
                                                                <button type="submit"
                                                                    class="w-full py-1.5 px-2 md:py-2.5 md:px-3 rounded-lg md:rounded-xl bg-gradient-to-r from-lib-secondary to-pink-600 text-white font-bold text-[10px] md:text-xs hover:shadow-lg hover:shadow-lib-secondary/40 transition-all flex items-center justify-center gap-1">
                                                                    <span>Buy</span>
                                                                    <span
    ...
    <script>
        ...
        function setupMobileSidebar() {
            ...
            if (!sidebar.querySelector('.sidebar-close-btn')) {
                ...
                if (p6) {
                    ...
                    closeSidebarBtn.addEventListener('click', () => {
                        ...
                        sidebar.classList.remove('fixed', 'inset-0', 'z-50', 'bg-lib-surf-dark', 'w-full');
            ...
            if (mobileFilterBtn) {
                mobileFilterBtn.addEventListener('click', () => {
                    ...
                    sidebar.classList.add('fixed', 'inset-0', 'z-50', 'bg-lib-surf-dark', 'w-full');
        ...
        function setView(mode) {
            ...
            if (mode === 'grid') {
                ...
                bookCards.forEach(card => {
                    const inner = card.querySelector('.bg-lib-surf-dark');
                    // Mobile: rounded-xl, Desktop: rounded-2xl
                    inner.className = 'bg-lib-surf-dark rounded-xl md:rounded-2xl overflow-hidden border border-white/10 hover:border-lib-primary/50 transition-all duration-500 hover:shadow-2xl hover:shadow-lib-primary/20 flex flex-col h-full hover:-translate-y-1';

                    // Reset Image container
                    const imgContainer = card.querySelector('[class*="aspect-"]');
                    if (imgContainer) imgContainer.className = 'relative aspect-[3/4] md:aspect-square overflow-hidden bg-gradient-to-br from-lib-surf-light to-lib-surf-dark';
                ...
                if (gridBtn) {
                    gridBtn.classList.add('bg-lib-primary', 'text-white');
                    gridBtn.classList.remove('text-gray-400', 'hover:text-white');
                }
                if (listBtn) {
                    listBtn.classList.remove('bg-lib-primary', 'text-white');
                ...
                bookCards.forEach(card => {
                    const inner = card.querySelector('.bg-lib-surf-dark');
                    inner.className = 'bg-lib-surf-dark rounded-xl md:rounded-2xl overflow-hidden border border-white/10 hover:border-lib-primary/50 transition-all duration-500 hover:shadow-2xl hover:shadow-lib-primary/20 flex flex-row h-auto hover:-translate-x-1';

                    // Adjust Image container for list view
                    const imgContainer = card.querySelector('[class*="aspect-"]');
                    if (imgContainer) imgContainer.className = 'relative w-24 md:w-48 shrink-0 overflow-hidden bg-gradient-to-br from-lib-surf-light to-lib-surf-dark';
                ...
                if (listBtn) {
                    listBtn.classList.add('bg-lib-primary', 'text-white');
                    listBtn.classList.remove('text-gray-400', 'hover:text-white');
                }
                if (gridBtn) {
                    gridBtn.classList.remove('bg-lib-primary', 'text-white');
        ...
        // Clear all filters
        document.querySelector('.text-xs.text-lib-primary')?.addEventListener('click', function () {
            activeFilters = {
                categories: [],
                minPrice: null,
                maxPrice: null,
        ...
        const modalsHTML = `
            ...
            <div id="authModal" class="fixed inset-0 z-[60] hidden">
                ...
                <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-lib-surf-dark border border-white/10 rounded-2xl p-6 shadow-2xl transform transition-all scale-100">
                    <div class="text-center">
                        ...
                        <div class="flex gap-3">
                            <a href="{{ route('login') }}" class="flex-1 py-2.5 bg-lib-surf-light hover:bg-white/10 text-white font-bold rounded-xl border border-white/10 transition-all text-sm">Log In</a>
                            <a href="{{ route('register') }}" class="flex-1 py-2.5 bg-gradient-to-r from-lib-primary to-lib-secondary text-white font-bold rounded-xl hover:shadow-lg hover:shadow-lib-primary/30 transition-all text-sm">Register</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="profileModal" class="fixed inset-0 z-[60] hidden">
                ...
                <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white dark:bg-lib-surf-dark border border-gray-200 dark:border-white/10 rounded-2xl p-6 shadow-2xl transform transition-all scale-100">
                    <div class="text-center">
                        ...
                        <a href="{{ route('profile.edit') }}" class="block w-full py-2.5 bg-lib-primary hover:bg-lib-primary-hover text-white font-bold rounded-xl shadow-lg hover:shadow-lib-primary/30 transition-all text-sm">
                            Go to Profile
                        </a>
                    </div>
                </div>
