        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Role: {{ $role->name }}</h1>
                        <p class="mt-2 text-gray-600 dark:text-gray-400">Update role details and permissions.</p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="p-8">
                    <form action="{{ route('library.admin.roles.update', $role->id) }}" method="POST" class="space-y-8">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required
                                {{ in_array($role->slug, ['admin', 'user']) ? 'readonly' : '' }}
                                class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all {{ in_array($role->slug, ['admin', 'user']) ? 'opacity-75 cursor-not-allowed' : '' }}"
                                placeholder="e.g. Content Moderator">
                            @if(in_array($role->slug, ['admin', 'user']))
                                <p class="text-xs text-gray-500 mt-1">Core role names cannot be changed.</p>
                            @endif
                        ...
                        <!-- Permissions -->
                        <div>
                            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4 block">Assign Permissions</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach($availablePermissions as $key => $label)
                                    <label class="relative flex items-start p-4 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
